...
impl<'a, F: FftField> EvalResult<'a, F> {
    ...
    fn init_<G: Sync + Send + Fn(usize) -> F>(
        ...
        // we skip evaluating them and leave them zero. The one exception is the
        // generic constraint, which equals the public input on the public-input
        // rows; rather than special-case it here, the prover adds the public
        // input back onto those rows after this skip (the public-input polynomial
        // added later in coefficient form then cancels it on d1). The d1 rows of
        // an evaluation over `res_domain.0` are the indices that are multiples of
        // the domain-size ratio (4 for d4, 8 for d8) -- a power of two, so we
        // test with a mask (`i & (stride - 1)`) rather than `%`, which the
        // compiler cannot lower to a mask given a runtime divisor. Results that
        // bypass this path get the same treatment via `zero_d1_rows`.
...
impl<F: FftField, Column: Copy> Expr<F, Column> {
        ...
        Environment: ColumnEnvironment<'a, F, ChallengeTerm, Challenge, Column = Column>,
    {
        ...
        let chunk = 1usize << 14; // multiple of B
                                  // 7n packing: for a d8 result every 8th row is a d1 row (a known zero for
                                  // the quotient), and since the chunk and block sizes are multiples of 8,
                                  // those rows are exactly lane 0 of every block. Start the lane loops at 1
                                  // to skip them -- 1/8 fewer column reads and bytecode steps. (d4 keeps
                                  // all lanes; its d1 handling is the public-row patch in the prover.)
