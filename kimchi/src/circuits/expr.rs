...
impl<'a, F: FftField> EvalResult<'a, F> {
    ...
    fn init_<G: Sync + Send + Fn(usize) -> F>(
        ...
        // we can skip evaluating them and leave them zero. We only do this in the
        // d8 domain: every degree-d8 constraint (and the permutation) is zero on
        // all of d1, but the generic (d4) constraint is non-zero on the
        // public-input rows -- it is cancelled there by the public-input
        // polynomial added later in coefficient form -- so it must be evaluated in
        // full. The d1 rows of a d8 evaluation sit at indices that are multiples
        // of 8; since that is a power of two we test with a mask (`i & 7`) rather
        // than `%`, which the compiler cannot lower to a mask given a runtime
        // divisor. Results that bypass this path get the same treatment via
