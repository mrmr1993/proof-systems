...
/// compression proof.
fn prove_pool_cache(
) -> &'static std::sync::Mutex<std::collections::HashMap<usize, Vec<std::sync::Arc<rayon::ThreadPool>>>>
{
...
pub(crate) fn with_prove_pool<R: Send>(f: impl FnOnce() -> R + Send) -> R {
    ...
    let pool = {
        ...
        cache.get_mut(&n).and_then(|free| free.pop()).unwrap_or_else(|| {
            std::sync::Arc::new(
    ...
    prove_pool_cache().lock().unwrap().entry(n).or_default().push(pool);
    result
}
